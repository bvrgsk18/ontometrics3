...
def build_simple_answer(data: list, user_question: str) -> str:
    ...
    try:
        ...
        if response.candidates and len(response.candidates) > 0 and response.candidates[0].content and len(response.candidates[0].content.parts) > 0:
            ...
            finish_reason = response.candidates[0].finish_reason if response.candidates else 'UNKNOWN'
            st.warning(f"Model returned no content for simple answer generation. Finish reason: {finish_reason}. Defaulting to empty string.")
...
def get_available_metadata_from_neo4j(metadata_type: str = "all", product_name: str = None) -> tuple[str, str, dict]:
    """
...
def classify_question_type(query: str) -> str:
    ...
    'METADATA_SERVICE_TYPE:PRODUCT_NAME' (for service types by product), or 'METADATA_AVAILABILITY'
    (for general data existence queries).
    """
    if not client:
        return "LLM client not initialized. Cannot classify question."
    prompt = f"""
    Classify the following user query as either "SIMPLE", "REASONING", "METADATA_SERVICE_TYPE:PRODUCT_NAME", or "METADATA_AVAILABILITY".
    If the query asks for service types for a specific product, extract the product name and return "METADATA_SERVICE_TYPE:[PRODUCT_NAME]".
    If the query asks about the existence or availability of data for a specific period, product, or general data, return "METADATA_AVAILABILITY".
    ...
    Query: "How many wireless ned adds for q1 2024 vs q2 2025?"
    Classification: SIMPLE
    ...
    Query: "What service types we have for wireline business?"
    Classification: METADATA_SERVICE_TYPE:WIRELINED

    Query: "Do we have data for january month for wireless?"
    Classification: METADATA_AVAILABILITY

    Query: "Do you have any data for 2023?"
    Classification: METADATA_AVAILABILITY

    Query: "Do we have wireless data?"
    Classification: METADATA_AVAILABILITY
    ...
    try:
        ...
        if response.candidates and len(response.candidates) > 0 and response.candidates[0].content and len(response.candidates[0].content.parts) > 0:
            ...
            if classification in ["SIMPLE", "REASONING", "METADATA_AVAILABILITY"]:
                return classification
            # New classification to handle "what service types for X product"
            elif classification.startswith("METADATA_SERVICE_TYPE_"):
                # Extract product name from the classification, e.g., "METADATA_SERVICE_TYPE_WIRELINED" -> "wireline"
                product_name_from_classification = classification.split("METADATA_SERVICE_TYPE_")[-1].lower().replace("ed", "") # Removes 'ed' if present
                return f"METADATA_SERVICE_TYPE:{product_name_from_classification}"
            else:
...
def _parse_and_call_metadata_checker(metadata_params_raw: Union[str, dict]):
    ...
    return get_available_metadata_from_neo4j(
        ...
        with st.spinner("Processing your query..."):
            try:
                ...
                if agent_decision == "IRRELEVANT":
                    ...
                    current_answer, cypher_query, cypher_metrics = get_available_metadata_from_neo4j(
